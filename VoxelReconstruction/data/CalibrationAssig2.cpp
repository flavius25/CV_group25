...
double iterativeCalibration(double epsilon);
...
int noImagesUsed;
int minImages = 30;
double epsilon_values[4] = {0.755, 0.133, 0.182, 0.172};
...
int main() {
  ...
  for (int i{1}; i<5; i++){
    ...
    std::string path = "./cam" + number + "/cam" + number + "_images/*.png";

    //image vector and reset global parameters 
    images.clear();
    maxIterationsReached = false;
    iterationsDone = false;

    //set epsilon value differently depending on information from trial runs
    double epsilon = epsilon_values[i-1];

    cv::glob(path, images);
    noImagesUsed = images.size();

    //getting the first overall rms reprojection error
    double rmsRP_Error = iterativeCalibration(epsilon);
    ...
    *continue looping through and removing images with highest perViewError */
    ...
    std::string filename = "cam" + number + "/intrinsics.xml";
...
double iterativeCalibration(double epsilon) {
  ...
  for(int i{0}; i<CHECKERBOARD[1]; i++)
  {
    for(int j{0}; j<CHECKERBOARD[0]; j++)
      objp.push_back(cv::Point3f(11.5 * j, 11.5 * i, 0));
  }
  ...
  for(int i{0}; i<images.size(); i++)
  {
    ...
    */
    if(success)
    {
      ...
      if (iterationsDone) {
      // Displaying the detected corner points on the checker board - uncomment next three lines if you wanna see images
