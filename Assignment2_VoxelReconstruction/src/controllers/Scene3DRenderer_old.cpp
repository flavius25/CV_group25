		...
		Camera* camera)
{
	...
	Mat hsv_image;
	int count = camera->getFramesAmount();
	...
	//resize(hsv_image, hsv_image, Size(644, 486));

	Ptr<BackgroundSubtractor> pMOG2;
	pMOG2 = createBackgroundSubtractorMOG2(500, 16.0, true);



	Mat img, foregroundMask, backgroundImage, foregroundImg;

	//int train = camera->getFramesAmount();

	//Mat frame, fgMask, fmask, fgMaskMOG2;

	//if (frame_count == train) { learning_rate = 0; } //stop learning after training



	pMOG2->apply(hsv_image, foregroundMask);


	//imshow("FORMASK",foregroundMask);
	...
	//resize(foregroundImg, foregroundImg, Size(644, 486));
	...
	//	//do nothing. Training ready.
	//}


	//update the background model
	//pBackSub->apply(hsv_image, fgMask);
	...
	//m_h_threshold, m_s_threshold and m_v_threshold are calculated manually , they have to be automatically!. implement it here.
	...
	//absdiff(channels[2], camera->getBgHsvChannels().at(2), tmp);
	//threshold(tmp, foreground, 30, 255, CV_THRESH_BINARY);


	//TODO post-processing - apply erosion , dilation maybe on foreground 
