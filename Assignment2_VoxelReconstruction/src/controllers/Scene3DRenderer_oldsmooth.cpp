		...
		Camera* camera)
{
	...
	//background learning
	m_video = VideoCapture(camera->getDataPath() + General::BackgroundVideoFile);
	...
	Mat hsv_image;
	cvtColor(camera->getFrame(), hsv_image, COLOR_RGB2GRAY);  // from BGR to HSV color space

	//update the background model
	pBackSub->apply(hsv_image, fgMask, learning_rate=0);
