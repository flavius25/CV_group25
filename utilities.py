...
def binarizeLabels(train_label:list, test_label:list):
...
def checkTrainability(model):
    i = 0
    for layer in model._layers:
        if layer.trainable is False:
            print(f"Layer {i} is not trainable")
        else:
...
def loadSF40(img_size=(224,224)):
    ...
    train_labels, test_labels = binarizeLabels(train_labels, test_labels)
    
    return (SF_training_set, train_labels, SF_test_set, test_labels)



def createOFData(video_set, img_size = (112,112)):
    OF_data = []
    ...
    for video_path in video_set:
        ...
        cap = cv2.VideoCapture(f'TVHI_data/tV_human_interactions_videos/{video_path}')
        
        frame_count = cap.get(cv2.CAP_PROP_FRAME_COUNT)
        start_frame = int(frame_count/2)
        
        cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame) #Get the frame in the middle of the video.
        ...
        OF_data.append(np.asarray(video_OF_frames))
...
def loadTVHIData(img_size=(224,224)):
    ...
    TVHI_training_set = []
    for video_path in set_2:
        cap = cv2.VideoCapture(f'TVHI_data/tV_human_interactions_videos/{video_path}')
        frame_count = cap.get(cv2.CAP_PROP_FRAME_COUNT)
        cap.set(cv2.CAP_PROP_POS_FRAMES, int(frame_count/2)) #Get the frame in the middle of the video.
        success, image = cap.read()
        if success:
            image = cv2.resize(image, img_size)
            TVHI_training_set.append(image)
            
    TVHI_test_set = []
    for video_path in set_1:
        cap = cv2.VideoCapture(f'TVHI_data/tV_human_interactions_videos/{video_path}')
        frame_count = cap.get(cv2.CAP_PROP_FRAME_COUNT)
    ...
    train_labels, test_labels = binarizeLabels(set_2_label, set_1_label)
...
def opticalFlowInput():
    ...
    training_data = createOFData(set_2)
    testing_data = createOFData(set_1)
