...
void Glut::drawVoxels()
{
	...
	//glVertex2f((GLfloat)center_circle.at<float>(0, 0), center_circle.at<float>(0, 1));
	//Scene3DRenderer& scene3d = m_Glut->getScene3d();
	//Mat center_circle = m_Glut->getScene3d().getReconstructor().getCenters();

	//cv::Scalar colorCircle1(0, 0, 255);
	//cv::Point centerCircle1(center_circle.at<float>(0, 0), center_circle.at<float>(0, 1));

	//int index_camera = m_Glut->getScene3d().getCurrentCamera();
	//Mat frame = m_Glut->getScene3d().getCameras()[index_camera]->getFrame();

	//cv::circle(frame, centerCircle1, 30, colorCircle1, 2);
	//cout << center_circle;
	glEnd();
	glPopMatrix();
}
